...
namespace YOLOv8WithOpenCVForUnity.UnityIntegration.Worker.DataStruct
{
    ...
    public readonly struct PoseEstimationCOCO17Data
    {
        ...
        private readonly float[] _rawLandmarks;
        ...
        public PoseEstimationCOCO17Data(float x1, float y1, float x2, float y2, float confidence, int classId, Vec3f[] landmarks)
        {
            ...
            _rawClassId = classId;
            _rawLandmarks = new float[LANDMARK_ELEMENT_COUNT];
            for (int i = 0; i < LANDMARK_VEC3F_COUNT; i++)
            {
                int offset = i * 3;
                _rawLandmarks[offset + 0] = landmarks[i].Item1;
                _rawLandmarks[offset + 1] = landmarks[i].Item2;
                _rawLandmarks[offset + 2] = landmarks[i].Item3;
            }
        }

        public readonly ReadOnlySpan<Vec3f> GetLandmarks()
        {
            return MemoryMarshal.Cast<float, Vec3f>(_rawLandmarks.AsSpan());
        }
        ...
        public readonly override string ToString()
        {
            ...
            sb.Append("PoseEstimationCOCO17Data(");
